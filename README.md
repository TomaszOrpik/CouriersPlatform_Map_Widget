...
-  Testy można przeprowadzić w różnych środowiskach przekazując do komendy flagi `firebase` (do testów z wykorzystaniem firebase websocket), `headless` (uruchamia test w tle) oraz `production` (do testów z wykorzystaniem własnego websocketa i bazy danych mongoDB)
